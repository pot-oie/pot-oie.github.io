...
6. AI architecture abbreviations and model names such as `CNN`, `GNN`, `RNN`, `LSTM`, `GRU`, `ResNet`, `U-Net`, `Transformer`, `GPT`, `BERT`, `LoRA`, `Diffusion`, and `DiT` can stay in English, while article-specific mechanisms should prefer concise Chinese tags such as `卷积`, `消息传递`, `聚合函数`, or `解码策略`.
