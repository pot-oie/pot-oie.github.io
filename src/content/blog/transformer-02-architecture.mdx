...
这就是 FFN 存在的意义。严谨地说，这是一个 Position-wise（逐位置）的前馈网络。原论文中使用了两层线性变换和一个 ReLU 激活函数：

$$
\text{FFN}(x) = \max(0, xW_1 + b_1)W_2 + b_2
$$

它通常会先把特征维度放大 4 倍（由 $W_1$ 负责），经过激活函数引入非线性后，再缩小回原来的维度（由 $W_2$ 负责，例如 $512 \to 2048 \to 512$）。这个过程为网络提供了**巨大的参数空间和非线性表达能力**。
...
- **原始架构（Encoder-Decoder）**：Transformer 最初是为机器翻译任务量身定制的条件生成模型，完美结合了理解与生成的双重需求。
...
- **Encoder-Decoder 都保留**：继续沿用原汁原味的架构，将所有 NLP 任务统一转化为“Text-to-Text”（输入文本，输出文本）的形式，这不仅保留了条件生成任务（如翻译、摘要）上的绝对优势，也探索了大一统的范式，典型代表是 Google 的 **T5**。
