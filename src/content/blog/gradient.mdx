...
> 在给文章选择 HeroImage （头图）时，发现网上找的图片 or 生成的图片清晰度不够，跟其他元素的对比会很明显。
>
> 于是想到给图片附上边缘渐变消失的方式，通过“打码”的效果来规避清晰度的问题。
>
> 说着简单，结果发现这个小视效几个 AI 都生成不好，最后还是回归传统手艺，古法研制。

### 初步尝试

1. `linear-gradient` 的原理是在图片上层覆盖一个渐变蒙版，
通过调整蒙版的透明度来实现图片的渐变消失效果。

2. 重点在于确定希望展示的视觉中心，因为是文章的头图，需要兼顾图片信息和标题之上的时间戳，
所以选择图片的中间偏上位置作中心（40%）。

3. 因为 HeroImage 的父容器是 main 元素，其宽（800px）远大于图片宽度（高度 1:1 填充），
...
经过一个假期的修整，这个问题实现了变相的解决：与其辛辛苦苦调整蒙版数值，不如从根源解决问题——给头图一个新的展现方式、

我目前的打算是把 HeroImage 改为透明度偏高的背景图，这样就规避了清晰度问题，也不会有容器被压缩的问题。

但是实践（开始写博客）后才发现，配图其实是一件挺费精力的事，所以这个方案什么时候投入实践要取决于我什么时候对某篇文章的配图突然燃起热情了。