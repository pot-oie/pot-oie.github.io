...
问题很明显： `set` 的插入和遍历都是 $O(\log n)$，所以这里需要使用 $O(n)$ 的 `unordered_set` 。
...
![result](../../assets/blog/content/leetcode-128.png)

这是哈希表本身的劣势：

- **动态内存分配：** `vector` 作为动态数组，在数据转移时无法预知大小，这会导致建立哈希表时会进行多次扩容和重哈希，这是很耗时的操作。
- **哈希函数计算：** 虽然查表的理论时间复杂度是 $O(1)$，但实际上每次函数的计算映射隐形开销也是不小的成本。
...
![improve_result](../../assets/blog/content/leetcode-128-1.png)
