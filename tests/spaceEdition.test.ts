...
test("Space edition resolves complete random-refresh candidate pools", async () => {
  ...
  assert.equal(edition.randomSelection.filmCount, 8);
  assert.equal(edition.randomSelection.trackCount, 8);
...
test("Space edition rejects malformed film and music references", async () => {
  const watches = sources().watches.slice(0, 8);
  watches[0] = { ...watches[0], data: { ...watches[0].data, rating: undefined as never } };
  await assert.rejects(() => resolve({ watches }), /at least 8 eligible films/);

  const tracks = sources().tracks.slice(0, 8);
  tracks[0] = track("track-0", "");
  await assert.rejects(() => resolve({ tracks }), /at least 8 eligible tracks/);
});
